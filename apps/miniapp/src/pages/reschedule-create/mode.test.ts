...
describe("leader reschedule entry modes", () => {
  ...
  it("keeps ordinary same-week only while handbook review includes both date relations", () => {
    const targets = [
    ...
    expect(targetsForEntryMode(targets, "cross_week")).toEqual(targets);
  });
  ...
  it("states the handbook and administrator decision boundary in the cross-week copy", () => {
    ...
    expect(RESCHEDULE_ENTRY_COPY.cross_week.guidance).toContain("本周或跨周");
    expect(RESCHEDULE_ENTRY_COPY.cross_week.guidance).toContain("无需重新提交");
  });
});
