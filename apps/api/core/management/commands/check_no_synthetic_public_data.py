...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        demo = Season.objects.filter(
            id__in=AdminAuditLog.objects.filter(action=DEMO_AUDIT_ACTION).values("object_id")
        )
        if demo.exists():
            names = "、".join(demo.values_list("name", flat=True))
            raise CommandError(f"仍含合成演示赛季数据：{names}")
        leaderboard = Season.objects.filter(
            id__in=AdminAuditLog.objects.filter(action=LEADERBOARD_AUDIT_ACTION).values("object_id")
        )
        if leaderboard.exists():
            names = "、".join(leaderboard.values_list("name", flat=True))
            raise CommandError(f"公开赛季仍含合成榜单数据：{names}")
        media = Season.objects.filter(
            id__in=AdminAuditLog.objects.filter(action=MEDIA_AUDIT_ACTION).values("object_id")
        )
        if media.exists():
        ...
        self.stdout.write(
            self.style.SUCCESS("No synthetic public demo data or duplicate group photos found.")
        )
