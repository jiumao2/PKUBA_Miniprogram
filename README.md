...
当前小程序使用自定义大字号底栏：首页、对阵、排名、数据、我的；“我的”按权威待处理任务数显示 1–99 或 `99+` 红点，打开任务不会提前消除。首页在 Logo 与赛季标题下首先展示首场至末场的七列赛季热图，包含全部 0 场日期。“对阵”页内容第一行固定“赛程赛果 / 淘汰赛”切换栏；长赛程滚动后仍可直接切换。淘汰赛按男甲、男乙、女甲、女乙和唯一轮次键展示正式比赛中的双方、比分、胜者、日期、场地与保级赛；未录入显示“待抽签”，签位不一致保留原球队并标记“待复核”。赛程与淘汰赛分别请求，一侧失败不隐藏另一侧。若开发者工具仍显示系统默认小字号底栏，执行“清缓存 → 清除全部缓存”，重新点击编译，并确认 `dist/app.json` 中 `tabBar.custom` 为 `true`。

管理网站默认显示微信扫码登录：管理员先在小程序“我的”中完成微信身份识别，再点击“扫码登录管理后台”，扫描网页二维码并核对六位校验码。二维码五分钟失效，只能由生成它的原浏览器使用一次；管理员确认时仍会由服务端重新检查账号状态。也可切换到“密码登录”：本地网页登录名默认为 `local-admin`，密码保存在仅限本机的 `.env.wsl.local`；小程序管理员首次注册后的网页密码与注册时的邀请码相同，登录后可用右上角“修改密码”设置个人密码。新密码仅要求至少 4 个字符，可以与当前密码相同，不执行其他强度限制；邀请码轮换不会改变已有管理员密码。
...
调赛创建、记录表发布、赛程确认、抽签确认、赛季公开和比赛资料上传支持标准 `Idempotency-Key`。共享客户端会为新命令自动生成键；网络超时后若自行重试，必须复用原键，不能把同一业务当作新命令发送。公开赛程、领队调赛申请、管理端比赛资料和记录表队列均采用 `{ items, total, page, page_size }` 分页契约，单页最多 100 条。完整规则和已覆盖端点见 `docs/API_RELIABILITY.md`。
...
在首页、对阵、领队赛程或管理员赛程中点击一场比赛，可进入“比赛资料”。普通管理员与超级管理员可上传记录表、比赛合照和其他照片；非记录表图片的审核、替换后的管理和软删除仅超级管理员。记录表首次发布前两类管理员均可重传，发布后仅超级管理员可重传或纠错。领队只能查看本队比赛当前已发布版本绑定的记录表原图，公众不能读取私有原图。三类图片都接受可正常解码的 JPEG/PNG/WebP，以临时文件分块上传，不设置应用层字节或固定像素门槛；Pillow 解压安全保护、600 秒请求时限、客户端和磁盘仍是实际边界。记录表上传时还需要确认已正确结表。每场只有一张当前记录表原图，重传保留旧来源与审计。
