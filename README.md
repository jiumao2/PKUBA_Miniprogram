...
管理网站默认显示微信扫码登录：管理员先在小程序“我的”中完成微信身份识别，再点击“扫码登录管理后台”，扫描网页二维码并核对六位校验码。二维码五分钟失效，只能由生成它的原浏览器使用一次；管理员确认时仍会由服务端重新检查账号状态。也可切换到“密码登录”：本地网页登录名默认为 `local-admin`，密码保存在仅限本机的 `.env.wsl.local`；小程序管理员首次注册后的网页密码与注册时的邀请码相同，登录后用右上角“修改密码”设置个人密码。邀请码轮换不会改变已有管理员密码。
