...
服务器完成一次性受限部署账号和 GitHub `production` Environment 配置后，日常发布只需保证本地 `main` 干净且与 `origin/main` 一致，然后运行：

```powershell
./scripts/release.ps1 -Version v0.3.0
```

标签会依次触发完整 CI、不可变 GHCR 镜像、小程序生产构建、服务器备份/维护/迁移/验收和失败自动回滚；只有生产验收成功才创建 GitHub Release。首次服务器接入、Secrets、回滚点与人工灾难恢复步骤见 [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md)。
