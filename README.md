...
当前小程序使用自定义大字号底栏：首页、对阵、排名、数据、我的。首页在 Logo 与赛季标题下首先展示首场至末场的七列赛季热图，包含全部 0 场日期。“对阵”页内容第一行固定“赛程赛果 / 淘汰赛”切换栏；长赛程滚动后仍可直接切换。淘汰赛按男甲、男乙、女甲、女乙展示真实轮次、比分、胜者和保级赛。若开发者工具仍显示系统默认小字号底栏，执行“清缓存 → 清除全部缓存”，重新点击编译，并确认 `dist/app.json` 中 `tabBar.custom` 为 `true`。
...
领队工作台已提供本队比赛、发起普通/跨周调赛、查看申请、对手确认、指定球队投票、撤回和线下特殊调赛说明。管理员工作台已提供调赛审核与取消、赛程查看及记录表核对；超级管理员还可二次确认后直接纠错赛程或已发布记录表，并显式处理关联申请和审计记录。旧小程序的裁判功能不在 V1。

管理网站“调赛处理”只展示当前公开赛季。普通管理员和超级管理员可以筛选进行中或历史申请，核对比赛活动锁、永久可调政策、目标场地预留和时段容量后处理跨周直批、拒绝、指定球队投票、终审及取消。所有按钮只提交申请 ID、版本与命令，状态迁移、比赛移动和资源释放仍由 Django/PostgreSQL 事务决定。
...
在首页、对阵、领队赛程或管理员赛程中点击一场比赛，可进入“比赛资料”。记录表、比赛合照和其他照片都只允许普通/超级管理员上传；领队只能查看本队比赛当前已发布版本绑定的记录表原图，公众不能读取私有原图。记录表接受可正常解码的 JPEG/PNG/WebP，单张不超过 20 MB，不设固定像素门槛，并需要确认已正确结表。每场只有一张当前记录表原图，重传保留旧来源与审计；比赛合照和其他照片继续走管理员审核、退回、替换和软删除流程。

记录表上传后由独立 PostgreSQL worker 调用 Qwen，首次失败后最多再自动重试三次。网页和小程序共享唯一草稿、版本、核对状态、校验报告、publication 和 60 秒编辑租约，以两秒轮询同步；发布事务同时写正式比分、不可变球队/球员统计和审计。公开“数据”页只展示纸质记录表能够提供的已发布统计。完整协议、隐私边界、重试、编辑器、导出与备份见 `docs/SCORESHEETS.md`。
...
旧 `ScoresheetReader` 保持只读；本仓库已按当前 Django/Taro/React 架构重新实现结构化记录表、跨端核对、赛果发布和统计，不在运行时调用或修改旧项目。
