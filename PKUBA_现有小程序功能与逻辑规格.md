...
- 记录表原图、比赛合照和其他照片按稳定 `gameId` 隔离并明确分类。普通管理员与超级管理员可上传资料；非记录表图片审核与删除仅超级管理员。记录表首次发布前两类管理员均可重传，发布后仅超级管理员可重传或纠错。领队只能查看本队比赛当前已发布版本绑定的记录表原图；公众只能读取已发布结构化统计，不能读取私有原图。服务端必须安全解码图片并记录真实尺寸，但不设固定分辨率门槛；所有替换、删除、审核和发布均留痕。
...
- 旧小程序和 `ScoresheetReader` 仓库保持只读。电脑端以 `ScoresheetReader` 提交 `c27fe987a71637a8f1161a991d3c15f6c0afdeed` 为冻结基线，完整迁移原界面、编辑方式、领域模型、FIBA 2024 校验、单页 A4 模板坐标及 PDF 渲染；PKUBA 只替换认证、数据、自动租约和同步适配。小程序按手机操作设计无损适配，Django/PostgreSQL 是唯一业务权威。
...
- 每场比赛只有一个当前 `GameScoresheet`。仅超级管理员可重传并建立新的来源版本，使旧识别任务 `SUPERSEDED`；旧来源、revision、publication 和审计记录不删除，迟到识别结果不得覆盖新来源或已有人工修改。
...
- 发送给 Qwen 的先验仅包含处理后的整表图片、双方球队名称和球员显示名称；不得发送账号、OpenID、UUID、球衣号码、赛季、场地或其他身份/赛程元数据。
...
- 抽签映射是超级管理员网页功能，只处理 `group` 非空的小组赛/循环赛初始签位；淘汰赛占位不在此页填写。球队名称必须从当前组别的启用 `Team` 数据中选择，不允许自由输入、跨组选择、停用球队或同一球队重复出现。
- 一个组别的启用球队数必须等于初始签位数，并且每次保存都提交该组别的完整一对一映射。客户端实时从其他下拉栏移除已选球队；服务端仍以稳定 UUID、精确签位/球队集合和数据库唯一约束作最终校验。
- 保存采用“影响预览—确认写入”：预览返回签位变化、受影响比赛、公开影响、阻塞原因和绑定版本状态的哈希。确认时服务端锁定相关对象并重新分析，在单一事务中更新 `DrawAssignment`、同步 `Game.home_team/away_team`、增加版本和追加审计；并发变化或任一失败都回滚。
- `SETUP`、`PRE_DRAW_PUBLIC` 可正常落位；`ACTIVE` 只允许修正未来且仍为 `SCHEDULED`、无比分、无任何调赛记录和比赛资料的相关比赛。`ARCHIVED` 只读，更高风险历史纠错不在本页处理。
...
重写规则：媒体必须按稳定 `gameId` 关联，并分为记录表、比赛合照、其他照片三类；页面固定按此顺序展示。普通/超级管理员可上传全部类别；领队不能上传，只能查看本队比赛当前 publication 绑定的记录表原图；公众不能访问私有原图。记录表仅接受服务端可解码的 JPEG/PNG/WebP 原图，单张不超过 20 MB，不设固定像素门槛。每场只有一个当前记录表来源；重传创建新来源版本，旧文件退出当前编辑入口但保留 publication 读取能力与审计。比赛合照和其他照片继续由管理员审核、退回、替换和软删除。文件保持私有，访问使用短期签名链接；结构化记录表的识别、核对、发布和统计规则见 A.10 与 `docs/SCORESHEETS.md`。
...
- [x] 超级管理员可创建准备中赛季，选择系统默认配置或仅沿用历史赛季的组别、启用场地、时段和容量；准备期配置以版本化单事务保存；组别可独立上线，归档为不可逆只读终态。
- [ ] 超级管理员可下载按实际组别生成的名单模板，上传审计球队/球员/号码和名称一致性，在无下游引用的准备中赛季原子确认一次整季快照；确认后只通过在线编辑维护，进行中赛季危险修改二次确认，归档只读。
- [x] 公开查看完整赛程、过去/未来分组、比分与地点。
- [x] 按大组/小组查看积分矩阵，并按相互积分、相互净胜分、相互总得分、总净胜分、总得分排名；不提供跨组排名。
- [x] 展示淘汰赛树：超级管理员确认权威胜者关系，后续胜者自动晋级；纠错先预览，再显式重置下游。
...
- `Division.operation_status` 与全局赛季状态分离；未激活组别禁止调赛、记录表和赛果，归档赛季及其全部业务不可恢复或修改。
- 淘汰赛下游位置只能由唯一 `GameWinnerFeed` 提供胜者；关系与赛果纠错都必须版本化、事务化并保留审计。
