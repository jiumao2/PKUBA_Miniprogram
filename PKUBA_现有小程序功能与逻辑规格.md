...
- 记录表原图、比赛合照和其他照片按稳定 `gameId` 隔离并明确分类。普通管理员与超级管理员可上传资料；非记录表图片审核与删除仅超级管理员。记录表首次发布前两类管理员均可重传，发布后仅超级管理员可重传或纠错。领队只能查看本队比赛当前已发布版本绑定的记录表原图；公众可读取已发布结构化统计和在线未删除的比赛合照，不能读取记录表或其他私有原图。服务端以临时文件分块接收、累计真实字节与 SHA-256，再安全解码图片并记录真实尺寸，不设应用层字节上限、固定分辨率门槛或自定义解码像素上限；Pillow 解压炸弹保护仍作为 413 安全错误。所有替换、删除、审核和发布均留痕。
...
- 小程序公开赛程以北京时间比赛日为游标，初次只返回焦点日前后最多 5 个比赛日，之后向上/向下各自增量加载，不再由客户端收集全季比赛；时间轴不插入“今天无比赛”、今日高亮、赛季边界或总场次说明，保留“回到今天”。首页、对阵、淘汰赛、数据单场、领队和管理员入口统一进入比赛详情：公众读取比赛、比分、当前 publication 统计及在线未删除的比赛合照，双方球员统计分别使用独立表格；比赛合照上传或管理员重新上传后立即公开且不受内部审核状态控制。记录表和其他照片继续私有，领队只读本队当前 publication 原图，管理员资料操作固定在详情页底部。
...
- 原图与比赛信息、A 队、B 队、逐次得分、节比分与最终结果、工作人员与签名六个区域只用于小程序导航、问题定位和识别差异，不设逐区人工确认门槛。发布前服务端 error 为零；全部 warning 只弹出一次确认，并要求原图/比赛/名单/草稿/校验报告均未变化、比分非平局且调用端仍持有租约。
...
- 小程序采用独立分包全屏原生编辑器，按区域顺序导航，并以单视图切换原图/标准表；逐次得分使用 1–40、41–80、81–120、121–160 四块纸面累计得分格。电脑端从比赛资料打开独立全视口页面，完整保留冻结版 `ScoresheetReader` 的所有原有编辑功能和操作方式。完整接口、冲突恢复、统计、导出和运维约定见 `docs/SCORESHEETS.md`。
...
- 记录表首次发布前普通管理员与超级管理员均可上传、重传、编辑、手工录入、四次耗尽后重试、校验与发布；活动识别不提供停止或重复启动。调赛审核/取消、资料审核/删除、强制租约以及已发布记录表纠错/重传/重新发布只创建给超级管理员的任务。
...
重写规则：媒体必须按稳定 `gameId` 关联，并分为记录表、比赛合照、其他照片三类。普通/超级管理员可上传全部类别；领队不能上传，只能查看本队比赛当前 publication 绑定的记录表原图；公众可通过短期签名链接查看在线未删除的比赛合照，但不能查看记录表和其他照片。全部类别仅接受服务端可解码的 JPEG/PNG/WebP 原图，使用安全临时文件分块接收，不设固定字节或像素门槛；600 秒请求时限、客户端能力、磁盘容量和 Pillow 解压炸弹保护仍是实际边界。每场只有一个当前记录表来源；重传创建新来源版本，旧文件退出当前编辑入口但保留 publication 读取能力与审计。比赛合照上传后立即公开，审核状态只供内部管理；超级管理员仍可审核、退回、排序或软删除。文件物理存储保持私有，访问使用短期签名链接；结构化记录表的识别、核对、发布和统计规则见 A.10 与 `docs/SCORESHEETS.md`。
...
| 26–27 技术统计照片 | 按 `gameId` 隔离；仅管理员上传，领队只读本队当前已发布记录表原图；记录表替换保留旧 publication，比赛照片支持审核、替换、软删除；文件与元数据一致性和审计纳入验收 |
...
- [ ] 管理员按 `gameId` 管理记录表和比赛图片；领队只能查看本队当前已发布记录表原图，公众可查看已发布统计和在线比赛合照；替换、排序、软删除、审核和 publication 来源均可审计。
...
- [ ] 电脑端完整保留冻结版 `ScoresheetReader` 的全视口工作台、编辑方式和全部功能；小程序全屏区域式编辑器可完整人工录入且不会丢弃未展开字段，纸面得分格覆盖 1–160、加分/删除/节末/终场与问题跳转，原图四角对齐只服务人工定位。
...
| 记录表与比赛资料 | 普通管理员与超级管理员可上传资料；首次发布前均可重传记录表、编辑、手工录入、四次耗尽后重新识别、校验与发布。活动识别不可停止或重复启动；已发布纠错/重传/重新发布、资料审核/删除和强制租约仅超级管理员。领队只读本队当前 publication 记录表原图，公众读取已发布统计和在线未删除的比赛合照 |
