...
- 记录表原图、比赛合照和其他照片按稳定 `gameId` 隔离并明确分类。普通管理员与超级管理员可上传资料；非记录表图片审核与删除仅超级管理员。记录表首次发布前两类管理员均可重传，发布后仅超级管理员可重传或纠错。领队只能查看本队比赛当前已发布版本绑定的记录表原图；公众只能读取已发布结构化统计，不能读取私有原图。服务端以临时文件分块接收、累计真实字节与 SHA-256，再安全解码图片并记录真实尺寸，不设应用层字节上限、固定分辨率门槛或自定义解码像素上限；Pillow 解压炸弹保护仍作为 413 安全错误。所有替换、删除、审核和发布均留痕。
...
- 新版数据页分为球队榜、球员榜和单场数据。球队榜从公开赛季正式比分计算并展示所选组别的完整榜单；球员榜固定每页 20 人、最多 5 页，仅展示所选排序下的前 100 名。球员统计只使用正常比赛当前 publication 中可由记录表得到且绑定稳定名单 ID 的出场、首发、总得分、场均得分、1/2/3 分事件和个人犯规。弃权不伪造球员记录，历史 publication 不重复累计；本阶段不提供球队或球员详情页。
...
重写规则：媒体必须按稳定 `gameId` 关联，并分为记录表、比赛合照、其他照片三类；页面固定按此顺序展示。普通/超级管理员可上传全部类别；领队不能上传，只能查看本队比赛当前 publication 绑定的记录表原图；公众不能访问私有原图。全部类别仅接受服务端可解码的 JPEG/PNG/WebP 原图，使用安全临时文件分块接收，不设固定字节或像素门槛；600 秒请求时限、客户端能力、磁盘容量和 Pillow 解压炸弹保护仍是实际边界。每场只有一个当前记录表来源；重传创建新来源版本，旧文件退出当前编辑入口但保留 publication 读取能力与审计。比赛合照和其他照片继续由管理员审核、退回、替换和软删除。文件保持私有，访问使用短期签名链接；结构化记录表的识别、核对、发布和统计规则见 A.10 与 `docs/SCORESHEETS.md`。
...
- [ ] 管理员按 `gameId` 管理记录表和比赛图片；领队只能查看本队当前已发布记录表原图，公众只能查看已发布统计；替换、软删除、审核和 publication 来源均可审计。
...
| 记录表与比赛资料 | 普通管理员与超级管理员可上传资料；首次发布前均可重传记录表、编辑、手工录入、四次耗尽后重新识别、校验与发布。活动识别不可停止或重复启动；已发布纠错/重传/重新发布、资料审核/删除和强制租约仅超级管理员。领队只读本队当前 publication 原图，公众只读已发布统计 |
