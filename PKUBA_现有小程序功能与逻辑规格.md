...
- 记录表原图、比赛合照和其他照片按稳定 `gameId` 隔离并明确分类。普通管理员和超级管理员可上传全部比赛资料；领队不能上传，只能查看本队比赛当前已发布版本绑定的记录表原图；公众只能读取已发布结构化统计，不能读取私有原图。服务端必须安全解码图片并记录真实尺寸，但不设固定分辨率门槛；所有替换、删除、审核和发布均留痕。
...
- 旧小程序和 `ScoresheetReader` 保持只读；只迁移后者的领域模型、FIBA 2024 校验、单页 A4 模板坐标及 PDF 渲染逻辑。新网页、小程序、API 和 worker 均在本仓库实现，Django/PostgreSQL 是唯一业务权威。
- 每场比赛只有一个当前 `GameScoresheet`。管理员重传会建立新的来源版本并使旧识别任务 `SUPERSEDED`，但旧来源、revision、publication 和审计记录不删除；迟到识别结果不得覆盖新来源或已有人工修改。
- 独立 PostgreSQL worker 调用 Qwen。总计最多四次尝试，即首次调用加三次自动重试；前三次失败后分别等待 30 秒、2 分钟和 10 分钟，服务商更长的 `Retry-After` 优先。网络、超时、429、5xx、无效 JSON 和 Schema 错误可重试；非法图片、名单缺失、凭据配置错误及已替换来源不可重试。四次耗尽后进入 `RECOGNITION_FAILED`，仍允许完整人工录入或重传。
- 发送给 Qwen 的先验仅包含处理后的整表图片、双方球队名称和球员显示名称；不得发送账号、OpenID、UUID、球衣号码、赛季、场地或其他身份/赛程元数据。
- 网页和小程序共享同一个草稿、单调版本号、六区核对状态、validation report、publication、识别状态和事件序列。本地仅保留未提交输入、视图位置与会话撤销记录；保存、核对、撤销/重做、校验和发布都写回服务端。
- 同一时刻只有一个客户端持有编辑租约。编辑端每 15 秒心跳，60 秒失联后过期；其他端以只读模式每 2 秒拉取增量。普通管理员不能覆盖有效租约；超级管理员二次确认后可强制接管，旧客户端后续写请求必须被拒绝。所有写请求同时校验账号、`client_id`、`surface`、`lease_token` 和 `expected_version`。
- 六个核对区域为原图与比赛信息、A 队、B 队、逐次得分、节比分与最终结果、工作人员与签名。识别不会自动核对；区域内字段变化会只清除该区域核对状态。发布前六区均已核对、error 为零、warning 逐项确认、原图/比赛/名单/草稿/校验报告均未变化、比分非平局且调用端仍持有租约。
- 发布事务创建不可变 publication、单场球队/球员统计和审计，同时更新正式比分与比赛状态并把来源标记为通过；任一步失败整体回滚。首次发布允许普通/超级管理员，已发布纠错、重传和重新发布仅超级管理员；新版本成功前，公众与领队继续读取上一 publication。
- 小程序采用独立分包全屏原生编辑器，按区域顺序审核，并以单视图切换原图/标准表；逐次得分使用 1–40、41–80、81–120、121–160 四块纸面累计得分格。网页采用全视口三栏工作台。完整接口、冲突恢复、统计、导出和运维约定见 `docs/SCORESHEETS.md`。
...
| `GameScoresheet` / `ScoresheetRevision` | game、当前来源、草稿 JSON、单调版本、识别/核对/校验状态、比赛与名单先验快照、不可变关键节点快照 | 每场至多一个当前记录表；来源替换和人工变更不得被迟到识别结果覆盖；六区摘要与草稿版本共同约束发布 |
...
- [ ] 记录表上传后由 PostgreSQL worker 调用 Qwen，首次失败后按 30 秒、2 分钟、10 分钟最多自动重试三次；成功停止、耗尽失败、主动停止、重传重置额度和迟到结果隔离均可验证。
- [ ] 网页和小程序共享唯一草稿、六区核对、校验报告、publication 与 60 秒单端编辑租约；双端 2 秒增量同步、心跳、主动交接、超级管理员强制接管和网络恢复冲突选择均可验证。
- [ ] 六区核对、error 为零、warning 逐项确认且全部输入摘要稳定后才能原子发布；发布同时更新 Game、不可变球队/球员统计和审计，失败完整回滚。
- [ ] 网页三栏工作台和小程序全屏区域式编辑器均可完整人工录入；小程序纸面得分格覆盖 1–160、加分/删除/节末/终场与问题跳转，原图四角对齐只服务人工定位。
...
| 记录表与比赛资料 | 全部资料仅管理员上传；领队只读本队当前 publication 的记录表原图，公众只读已发布统计。记录表由 Qwen 最多四次尝试、跨端租约审核、六区校验后原子发布；已发布纠错仅超级管理员；其他比赛照片保留审核、替换、软删除与审计 |
