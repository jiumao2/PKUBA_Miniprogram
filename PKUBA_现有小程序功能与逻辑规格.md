...
- 小程序公开赛程以北京时间比赛日为游标，初次只返回焦点日前后最多 5 个比赛日，之后向上/向下各自增量加载，不再由客户端收集全季比赛。首页、对阵、淘汰赛、数据单场、领队和管理员入口统一进入比赛详情：公众读取比赛、比分、当前 publication 统计及在线未删除的比赛合照；比赛合照上传后立即公开且不受内部审核状态控制。记录表和其他照片继续私有，领队只读本队当前 publication 原图，管理员资料操作固定在详情页底部。
