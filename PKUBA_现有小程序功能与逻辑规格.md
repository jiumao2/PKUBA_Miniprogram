...
容量属于 `SeasonMeta`。超级管理员只能在赛季 `SETUP` 准备阶段通过专用配置页修改；赛季公开后即锁定，普通管理员始终不能修改，超级管理员的单次赛程例外也不得改写容量元信息。V1 默认容量为：
...
- 记录表原图、比赛合照和其他照片按稳定 `gameId` 隔离并明确分类。任一当季领队和管理员可查看；本场参赛球队领队和管理员可上传，管理员审核、重新上传和在线删除，所有变更留痕。服务端必须安全解码图片并记录真实尺寸，但不设固定分辨率门槛；上传者确认已正确结表，该确认不能替代管理员人工审核。
...
- 工作簿固定为四个可见工作表：“填写说明”“赛制定义”“比赛清单”“赛程网格”，不含签名、隐藏赛季身份、HMAC 元数据或 `leaderAdjustable` 字段。服务器模板只提供按赛季预填的便利，符合 V2 结构的自备文件同样可以上传。
- 导入前数据库必须已有赛季、组别、球队、时段、容量和场地。XLSX 只可新增小组、签位和比赛，不创建或修改球队，不写抽签映射。
- 每次上传都生成暂存批次；确认前不写正式赛程。已有比赛编号、同代码结构冲突、清单/网格不一致或任何排期冲突都会阻止确认，未列出的旧比赛保持不变。
- 确认时锁定赛季与相关排期资源，复核文件 SHA-256 和并发版本，并在单一 PostgreSQL 事务中创建记录、写来源批次、增加赛季版本和审计。新比赛固定以 `leaderAdjustable=true` 创建。
- 误导入只能由超级管理员在赛季 `SETUP` 状态执行受保护的整批重置。抽签、已开始/有比分比赛、调赛申请或预留、媒体、非导入引用及不可追溯对象均为阻塞条件；重置只删除导入对象，保留手工对象和赛季基础配置。
...
重写规则：照片必须按稳定 `gameId` 关联，并分为记录表、比赛合照、其他照片三类；页面固定按此顺序展示。任一当季领队和管理员可查看，本场参赛球队领队和管理员可上传，管理员审核、重新上传和在线删除。记录表仅接受服务端可解码的 JPEG/PNG/WebP 原图，单张不超过 20 MB，不设固定像素门槛；上传者必须确认已正确结表，管理员仍须查看原图后通过或填写原因退回。替换后新图片重新待审核，旧文件退出在线列表但保留审计记录。文件保持私有，访问使用短期签名链接，元数据、审核、替换和删除均记录审计。
...
| `SeasonMeta` | `seasonId`、固定容量表、period 定义与本地时间、有序场地列表 | 仅超级管理员可在 `SETUP` 阶段配置；公开后锁定，赛程例外也不改写容量表 |
...
- [ ] 管理员按 `gameId` 管理记录表和比赛图片，领队和管理员可查看；支持上传、查看原图、删除、替换、排序和审计。
...
- 赛季容量、period 和场地顺序在 `SETUP` 阶段由超级管理员配置，公开后成为固定元信息；例外操作不改变它们。
...
| 技术统计照片 | 按 game ID 隔离，仅领队/管理员查看；管理员可上传、查看原图、删除、替换、排序并留痕，不设固定分辨率门槛 |
