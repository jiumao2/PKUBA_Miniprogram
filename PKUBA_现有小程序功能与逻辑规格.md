...
- 旧小程序和 `ScoresheetReader` 仓库保持只读。电脑端以 `ScoresheetReader` 提交 `c27fe987a71637a8f1161a991d3c15f6c0afdeed` 为冻结基线，完整迁移原界面、编辑方式、领域模型、FIBA 2024 校验、单页 A4 模板坐标及 PDF 渲染；识别图片准备与 Qwen 请求模式以提交 `d631601` 为基线。PKUBA 只替换认证、数据、自动租约和同步适配。小程序按手机操作设计无损适配，Django/PostgreSQL 是唯一业务权威。
...
- 小程序采用独立分包和微信原生导航栏，不自绘沉浸式全屏标题；逐次得分使用 1–40、41–80、81–120、121–160 四块纸面累计得分格，点击任意空格选择球衣号并由相邻累计格推导分值，允许中间插入和清空。暂停、犯规数与节比分用加减按钮，犯规使用 FIBA 2024 受限选择器；最终比分与胜队自动派生，工作人员仅填写可确认姓名。电脑端从比赛资料打开独立全视口页面，完整保留冻结版 `ScoresheetReader` 的所有原有编辑功能和操作方式，仅同步取消签名状态输入及派生最终赛果。完整接口、冲突恢复、统计、导出和运维约定见 `docs/SCORESHEETS.md`。
...
- [ ] 电脑端完整保留冻结版 `ScoresheetReader` 的全视口工作台、编辑方式和全部功能；小程序原生页面编辑器可完整人工录入且不会丢弃未展开字段，纸面得分格覆盖 1–160、任意格选号码、插入/清空、节末/终场与问题跳转，原图四角对齐只服务人工定位。
