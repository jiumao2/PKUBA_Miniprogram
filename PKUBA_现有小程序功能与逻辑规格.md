...
- 赛季状态只保留 `SETUP / PUBLISHED / ARCHIVED`，同一时间至多一个公开赛季；删除单独 `is_public` 和组别运营开关。是否可调赛、上传资料、维护记录表或赛果由赛季已公开、比赛双方已落位及对应业务条件派生，不再另设“组别开放”或“赛季开始”。
...
- 同一时刻只有一个客户端持有编辑租约。编辑端每 15 秒心跳，60 秒失联后过期；其他端以只读模式每 2 秒拉取增量。自动识别执行或等待重试时所有人只读；终态成功、四次耗尽或凭据缺失后，普通管理员与超级管理员均可在首次发布前取得租约并编辑。发布后普通管理员只读，纠错仅超级管理员。两端不显示接手、交接或强制接管按钮；旧客户端后续写请求必须被拒绝，所有写请求同时校验账号、`client_id`、`surface`、`lease_token` 和 `expected_version`。
...
- 赛季状态只保留 `SETUP / PUBLISHED / ARCHIVED`，不存 `is_public` 或 `Division.operation_status`；业务操作要求赛季已公开、比赛双方已解析并满足对应业务条件，归档赛季及其全部业务不可恢复或修改。
