...
- 网页和小程序共享同一个草稿、单调版本号、validation report、publication、识别状态和事件序列。本地仅保留未提交输入、视图位置与会话撤销记录；保存、撤销/重做、校验和发布都写回服务端。小程序没有展开的原表字段通过无损投影保留，不能因手机端保存而丢失。
...
- [ ] 电脑端保留冻结版 `ScoresheetReader` 的全视口工作台、画布与非派生字段能力；小程序原生页面编辑器可完整人工录入且不会丢弃未展开字段。双端纸面得分格覆盖 1–160、任意格只选/换/清空号码，分值、节次、符号与节末/终场自动派生并支持问题跳转；原图四角对齐只服务人工定位。
...
| 容量与场地 | 固定为赛季元信息；标准申请提交即预留一场容量和启用候选池中的第一个可用场地；默认三场地时，周末容量 2 可从中任取两个 |
