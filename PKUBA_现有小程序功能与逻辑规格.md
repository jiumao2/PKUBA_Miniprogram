...
| 跨周调赛 | 领队使用独立入口，只列跨自然周目标，并提示参阅《参赛手册》的跨轮次调整方式；是否属于跨轮次由管理员审核决定，未跨轮次按普通调赛办法处理；对手同意后管理员可直批或指定球队投票，按时完成球队确认后可无限期等待管理员终审 |
...
- 小程序中的“跨周”只表示日期跨越系统自然周，不直接等同于手册中的“跨轮次”；是否跨轮次由管理员依据手册审核，客户端和领队不能自行认定。
