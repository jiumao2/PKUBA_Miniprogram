...
- 小程序采用独立分包和微信原生导航栏，不自绘沉浸式全屏标题；逐次得分使用 1–40、41–80、81–120、121–160 四块纸面累计得分格，每格只允许选择、替换或清空球衣号。分值、节次、得分符号、序号和节末/终场标记由固定格位置及书面节比分自动派生，异常四分以上间隔可暂存但不得发布。暂停、犯规数与节比分用加减按钮，犯规使用 FIBA 2024 受限选择器；最终比分与胜队自动派生，工作人员仅填写可确认姓名。电脑端使用同一固定格派生规则，其余工作台、画布和字段能力继续保留冻结版 `ScoresheetReader` 设计。完整接口、冲突恢复、统计、导出和运维约定见 `docs/SCORESHEETS.md`。
