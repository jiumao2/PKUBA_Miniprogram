...
日常操作可靠性补充：已有名单球员在预览和确认之间保留稳定身份，UUID 的传输表示不改变目标快照；完整目标或版本变化须拒绝旧确认。管理站逐字段错误按字段和原因显示，不序列化原始输入或内部异常。小程序普通/跨周调赛创建和申请列表分别显示未登录、身份失效、权限不足、请求失败及真实空数据状态；登录成功仅回跳白名单内的原入口，合法申请深链接保留，取消或失败不跳转。失去会话或更换身份后清除旧私有内容，旧响应不能覆盖新身份。照片归档的 CSV、包内 manifest 与任务 JSON 使用相同历史时间表示；清单序列化失败不得提升为最终包或放行永久清理。相应操作见 `docs/USER_GUIDE.md`，归档细节见 `docs/BACKUP_AND_ARCHIVE.md`。

名单在线维护以完整最终名单为准：稳定球员 ID 不得重复提交；同一事务支持号码互换、循环和停用/省略后复用，保留原 ID、历史引用及启用号码唯一约束。版本或确认失效、最终重号及后续保存错误均零部分写，临时释放号码不产生独立业务版本或审计。管理站将已知字段、行号和常见内置校验类型呈现为中文；未知字段/类型使用安全兜底，不回显内部路径、输入或上下文，保留已有业务错误与状态语义。
...
跨周入口进入参赛手册审核通道，可选择同周或跨周目标；同周/跨周日期关系仍以**周一至周日**为一周边界：

```text
等待对手
└─ 对手按时同意 → 等待管理员决定
   ├─ 管理员认定“按普通办法” → 直接批准 → 通过（仅同周）
   └─ 管理员认定“跨轮次调整”
      ├─ 直接批准 → 通过
      ├─ 拒绝 → 拒绝
      └─ 指定球队确认 → 等待指定球队
...
- 淘汰赛、半决赛、决赛和保级赛的所有轮次都由超级管理员在“签位结果录入”按比赛手工录入双方；系统不假定只有一轮抽签，也不自动晋级。后续轮次以紧邻上一轮正式胜队作为动态校验，越级选择必须再次确认；赛果纠正只标记待复核，不替换或清空任何下游数据。
...
记录表来源权限补充：本场曾有任何正式 publication 后，即使超级管理员更换原图、当前记录又处于草稿，普通管理员仍不可重传。媒体 `can_replace` 和记录表队列/详情/同步 `can_upload_source` 由同一服务端规则派生，双端只显示获准入口；原图查看与合照/其他照片操作不受影响。管理站账户与赛程编排的窄屏表单/工具栏须留在视口内，宽表只在内部横滚，不用裁切整页掩盖溢出。

网页来源能力同步须隔离旧打开代次、旧事件和最近详情读取之前的响应，即使草稿版本/事件号相同也不能让旧权限回流。详情读取按请求先后及服务端水位更新缓存，等待租约后的旧续接不能覆盖新详情，同水位的新权限仍可生效。打开和重新载入立即同步原图区与对应缓存比赛行，切换比赛不恢复已知过时权限，也不改其他比赛权限。网页每次等待详情或心跳后、继续获取租约前须重验打开上下文，切离后不发起旧记录的新租约。

小程序详情 GET（包括原图加载失败后的自动读取）、轮询和编辑命令回包分别维护摘要水位、完整正文与权限观察顺序。摘要先到而保存正文后到时，成功正文须补齐人工稿和后续版本，不能用摘要所附旧正文回退已保存内容；旧正文也不得替换已取得的新正文。真正随后提交并推进水位的成功写入仍保留，但其旧 actor 能力不能覆盖较新 GET 或轮询的权限判断。已被新读取取代的旧轮询失败不显示过时错误，当前失败正常提示；后续合法授权仍可恢复入口。刷新不清空待保存人工稿，不改来源、publication 或既有租约/离页策略；网页的打开代次及旧租约请求隔离不外推为小程序已实现或原生已验收。

失败暂存只在重新通过当前权限、赛季、来源版本和图片校验后恢复，拒绝零暂存写，已成功重放不重复落库。
...
- 唯一上传协议为 V3.3（格式 `3.3.0`），工作簿固定为“填写说明”“签位定义”“赛程网格”三个可见工作表；前两页自动生成，第二页为“组别、场次信息、轮次、签位代码”四列提示，业务导入只读取带日期、星期和明确网格线的第三页。文件不含签名、隐藏赛季身份、HMAC 元数据或 `leaderAdjustable` 字段；旧版本不兼容也不转换。
- 赛季与组别页配置 `ScheduleSlotFamily` 的组别、阶段、轮次、大小写敏感字母和签位数；赛程编排使用独立版本化 `ScheduleGridDraft`，网格列保存标准时段、自由文本场地、顺序和是否仅限决赛。16 列只是可编辑的便捷初始排版，实际支持 1–64 个连续动态列；管理员填写 `A1vsA2`，女子比赛追加 `（女）`。
- 导入前数据库必须已有赛季、组别、球队、时段、容量和签位族；场地文字来自列头，不要求 `Venue`，也不创建场地。XLSX 只新增小组、签位和比赛，不创建或修改球队，不写抽签映射；正式比赛轮次采用数据库签位族的 `round_number`。
- 小组赛/循环赛签位族必须覆盖完整两两对阵；淘汰赛、半决赛、决赛和保级赛每个签位必须且只能出现一次。缺失、反向重复、阶段混用、球队数不匹配、决赛专用列误用或任何排期冲突都会阻止确认。
- XLSX 通过结构检查后整表替换服务器草稿；覆盖非空草稿须先确认，业务问题保留供在线修正。确认前不写正式赛程；确认时锁定赛季与相关排期资源，复核文件 SHA-256、容量和并发版本，在单一 PostgreSQL 事务中创建记录、写来源批次、增加赛季版本和审计。XLSX 新格默认 `leader_adjustable=true`，在线草稿可逐场修改。
- 误导入只能由超级管理员在赛季 `SETUP` 状态执行受保护的整批重置。抽签、已开始/有比分比赛、调赛申请或预留、媒体、非导入引用及不可追溯对象均为阻塞条件；重置只删除导入对象，保留手工对象以及签位族、在线草稿列和容量配置。完整协议见 `docs/SCHEDULE_IMPORT_V3.md`。
...
- 球队名、球员名必填。`RosterPlayer.jersey_number` 是文本，可空，只接受 `0–99` 或 `00`；同队启用球员的非空号码重复属于错误并阻止确认，同队重复球员姓名也阻止确认。
...
- 每场比赛只有一个当前 `GameScoresheet`。首次发布前普通管理员与超级管理员均可重传并建立新的来源版本；已发布后仅超级管理员可重传。新来源使旧识别任务 `SUPERSEDED`；旧来源、revision、publication 和审计记录不删除。迟到结果不得覆盖不同来源或入队确认之后的新修改；下述明确确认的失败重试，成功后会整稿覆盖入队前人工内容。
- 独立 PostgreSQL worker 调用 Qwen。总计最多四次尝试，即首次调用加三次自动重试；前三次失败后的默认等待均为 30 秒，服务商更长的 `Retry-After` 优先。网络、超时、429、5xx、无效 JSON 和 Schema 错误可重试；非法图片、名单缺失、凭据配置错误及已替换来源不可重试。活动周期不能由用户停止或重复启动；失败后普通管理员与超级管理员均可完整手工录入。只有未发布、当前来源最新周期为 `FAILED` 且前置条件满足，才允许在明确覆盖确认后发起新周期：成功整稿覆盖（包括此前人工编辑），再次失败保留原稿。`SUCCEEDED` 及已有任何 publication 的记录不允许重试，超管也不例外。入队保存不可变前稿，Worker 落稿再次锁内核对当前来源/周期/发布状态；覆盖后重新校验并重新确认相关身份绑定，不沿用旧校验或映射，不新增差异/分区应用 UI。
...
- 发布、识别重试、调赛创建和照片上传/替换在当前客户端会话内保留结果未知操作的原键与原内容。响应丢失不自动新建命令；发布只有读回与本次草稿版本和来源一致的 publication 才确认成功，否则保留错误并允许按原上下文重试。调赛可在候选变空后核对原申请；批量照片逐项显示成功/失败/未确认、刷新列表，只由用户重试剩余原文件。既有幂等、版本和权限保护不放宽，具体操作见 `docs/USER_GUIDE.md`。
...
- 签位结果录入是超级管理员网页功能，统一处理 `group` 非空的小组赛/循环赛初始签位，以及任意多轮淘汰赛、半决赛、决赛和保级赛。球队必须从当前组别的启用 `Team` 数据中选择，不允许自由输入、跨组选择或停用球队。
...
- `SETUP`、`PUBLISHED` 均可落位；真正更换已有赛果或业务数据比赛的参赛方仍受危险修改阻断。`ARCHIVED` 只读。带签位比赛不能通过通用赛程编辑器直接改球队，必须回到签位结果录入。
...
| `Game` | 稳定 `gameId`、`seasonId`、阶段与 `roundNumber`、主客签位、正式比赛双方与赛程字段、导入的 `leaderAdjustable`、`activeRescheduleRequestId`、`scheduleVersion` | 永久政策与活动锁分离；按 ID 修改；版本用于乐观并发控制；正式比分不可相等；有签位的比赛只能在签位结果录入中修改球队 |
| `RescheduleRequest` | request ID、game ID、原比赛快照、目标 date/period/venue、日期关系、处理通道、审核认定、提交与确认截止、参与球队及各自确认、当前状态、`reservationId`、创建/更新时间 | 服务端权威；日期关系由服务端计算且不能由客户端覆盖；一个活动申请只能对应一场被锁比赛和一个有效预留；状态迁移必须校验当前状态、版本与权限 |
...
7. **旧系统把日期关系直接当作处理流程**：同周自动生效、跨周才审核。新版已拆分日期关系与处理通道；普通入口同周申请自动生效，跨周入口的同周/跨周申请都进入管理员审核。
...
| 22–23 淘汰赛 | 所有轮次由超级管理员在签位结果录入逐场录入；上一轮胜队只做校验，禁止自动晋级、临时推导与级联清空；小程序只显示正式比赛和待复核状态 |
...
- [x] 展示多轮淘汰赛：超级管理员在签位结果录入逐场录入所有轮次，上一轮胜队只做校验；小程序直读正式比赛，未落位显示“待抽签”，不一致显示“签位待复核”，且不自动修改下游。
- [ ] 领队、普通管理员和超级管理员按微信身份注册/登录，权限全部由服务端验证。
- [ ] 管理网站支持浏览器绑定、五分钟过期且单次消费的小程序扫码登录，并保留受限流保护的个人密码登录；管理员停用后已确认但未消费的挑战也不能建立会话。
- [ ] 领队看到本队未来比赛和申请列表。
- [ ] 普通/跨周入口遵守导入政策、比赛未开始、无活动申请、较早比赛日的 D-3/D-2 截止和周一至周日边界；服务端分别保存日期关系、处理通道和审核认定。
- [ ] 申请提交事务同时创建 Request、锁原比赛并预留目标容量与具体场地；所有非终态持续持有两种资源。
- [ ] 只有普通通道的同周申请由对手按时同意后自动生效；跨周入口的同周/跨周申请均由管理员先分类，未跨轮次可直接按普通办法批准，跨轮次可直批、拒绝或指定球队确认，完成确认后可无限期等待管理员终审。
...
- 球衣号码以文本保存，空、`0–99` 和 `00` 合法；同队启用球员非空号码重号与同队重名均阻止确认。近似球队名称不得由服务端静默合并。
...
- 组别、球队、名单、赛程、时段、签位、调赛、记录表、统计、媒体、导入和归档引用均为赛季专属，不得跨季共用；数据库复合外键必须验证历史行，部署和恢复必须通过只读赛季一致性审计。
...
2026-08-27 补充的记录表发布决定：比赛内部版本、政策和活动锁变化不是单独的发布阻断条件；服务端在校验及发布事务中比较当前赛季/组别、球队稳定身份、日期/实际时间/标准时段、场地、阶段/轮次/小组和权威名单。实际变化先显示差异，由原权限内的管理员明确复核；原图和人工草稿保留，不默认重传。旧快照缺少关联依据只要求一次受控复核。确认绑定记录、source asset/version、draft version/digest、先验/名单及当前语义，10 分钟过期或发生二次变化必须重新校验。球队/球员身份变化不得自动把旧得分归给同名新身份，未解决名单映射继续阻断。上传来源、recognition、旧 publication/revision 保持不变，复核前后新增不可变修订和审计；归档 WEB 超管的逐记录既有 publication 纠错窄例外不扩大。协议与用户操作分别见 `docs/SCORESHEETS.md`、`docs/USER_GUIDE.md`。

| 主题 | 已确认结论 |
|---|---|
| 抽签 | 只保留规则文字；申请、证明和抽签均在线下邮件完成，不上传附件、不建系统审批流；超级管理员录入最终结果 |
| 普通调赛 | 领队使用独立入口，只列同一自然周目标；对手按时同意后由服务端自动验证并生效，不增加管理员终审 |
| 跨周调赛 | 领队使用独立入口，同时列出同周与跨周合法目标，并提示参阅《参赛手册》；该入口全部申请都进入参赛手册审核通道。对手同意后管理员先认定“按普通办法”或“跨轮次调整”；同周且未跨轮次可直接批准，无需重提，认定跨轮次后可直批、拒绝或指定球队确认，按时完成球队确认后可无限期等待管理员终审 |
...
| 淘汰赛 | 任意多轮均由超级管理员在签位结果录入逐场手工录入；上一轮胜队只用于校验，允许显式越级；赛果纠正只标记待复核，不自动写入或清空下游 |
...
- “同一自然周/跨自然周”只表示服务端计算的日期关系；“跨周调赛”入口表示参赛手册审核通道，并可包含同周目标。是否跨轮次由管理员依据手册审核，客户端和领队不能自行认定。
- 普通调赛由对手确认后自动生效，不再增加管理员终审。
- 跨周是否投票以及指定哪些球队由管理员裁量，不强制全部候选球队参与。有小组归属时候选限同小组；淘汰赛等无小组归属时，候选为同赛事组别内除比赛双方外的全部启用球队，不沿用主队原小组。
...
- 普通/跨周调赛的候选读取绑定当前选择的比赛；更换比赛后，旧请求的成功、失败和加载结束均不得覆盖当前状态，目标未就绪时不得提交。
- 任务箱的首屏和分页结果只归属发起时的当前筛选；切换后不混入旧行、旧游标或旧错误，也不由旧请求结束当前加载。
- 数据页自动刷新导致赛季/组别变化时，旧榜单和分页立即失效；同一组别前台刷新仍可显示已有数据，后续由服务端结果更新。
- 小程序共用记录表抽屉的内边距由 ScrollView 内层 View 承载，不改变编辑、关闭和只读行为。网页工作台启动与模板失败提示不假设本地运行环境，实际服务错误继续优先显示。
